...
class Config:
    def __init__(self):
        ...
        self.enc_in = 256                        # Encoder input dimension (example value)
...
class MoEMedformer(nn.Module):
    def __init__(self, input_dim=250, output_dim=250, num_experts=3, medformer_config=None, modalities=['eeg', 'meg', 'fmri']):
        """
        MoEMedformer 使用多个 Medformer 作为专家，并通过路由器分配权重。

        Args:
            input_dim (int): 输入特征的维度。
            num_experts (int): 专家的数量。
            medformer_config (dict): 初始化 Medformer 所需的配置参数。
        """
        ...
        if medformer_config is None:
            raise ValueError("medformer_config 必须提供以初始化 Medformer 专家。")
    ...
    def forward(self, x, modality):
        ...
        Args:
            x (torch.Tensor): 输入张量，形状为 [batch_size, num_latents, input_dim]
            modality (str): 当前输入的模态（例如 'eeg', 'meg', 'fmri'）

        Returns:
            torch.Tensor: 输出张量，形状为 [batch_size, num_latents, output_dim]
        """
        if modality not in self.modalities:
            raise ValueError(f"Unsupported modality '{modality}'. Supported modalities: {self.modalities}")

        batch_size, num_latents, input_dim = x.shape

        # 获取对应模态的 λ 参数，形状 [num_experts, output_dim]
        lambda_m = self.lambda_params[modality]  # [num_experts, output_dim]

        # 逐个专家计算输出并应用 λ 权重
        expert_outputs = []
        for i, expert in enumerate(self.experts):
            # 假设 Medformer 的输出形状为 [batch_size, num_latents, output_dim]
            expert_output = expert(x)  # [batch_size, num_latents, output_dim]
            
            # 获取对应专家的 λ 参数，形状 [output_dim]
            lambda_i = lambda_m[i]  # [output_dim]
            
            # 将 λ 应用于专家输出，使用广播
            weighted_output = expert_output * lambda_i  # 广播至 [batch_size, num_latents, output_dim]
            expert_outputs.append(weighted_output)
...
class UnifiedEncoder(nn.Module):
    def __init__(self, in_dim=250, h=1024, out_dim=250, num_latents=128, qformer_spec=cfg.hyperparameter):
        ...
        # 加入 MoE 投影模块
        # self.moe_projection = MoEProjection(input_dim=in_dim, output_dim=out_dim, num_experts=5)
        ...
        for modal in self.modals:
            if modal =='image':
                ...
                #     in_channels=201, out_channels=hid_width, kernel_size=50, bias=False)
                self.positional_embedding[modal] = nn.Parameter(torch.empty([modal_tokens, hid_width]))
                nn.init.normal_(self.positional_embedding[modal], std=0.02)     
                # self.projection[modal] = nn.Linear(modal_tokens, 64)     
                # 使用自适应平均池化层进行下采样
