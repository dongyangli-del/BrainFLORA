...
class FusionHead(nn.Module):
    ...
    def init_cross_attn(self, qformer_spec, modality: str):
        """ initialize cross attention module and the learnable tokens """
        token_num = getattr(qformer_spec.crossattn_latent, modality)
        self.tokens = nn.Parameter(
            torch.randn(1, token_num, qformer_spec.modality_embed_dim) * INIT_CONST
        )

        self.cross_attention = CrossAttention(
            qformer_spec.modality_embed_dim,
            heads=qformer_spec.crossattn_heads,
            dim_head=qformer_spec.crossattn_dim_head,
            dropout=qformer_spec.crossattn_modality_dropout,
        )
