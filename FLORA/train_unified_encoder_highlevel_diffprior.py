...
def train_model(epoch, unified_model, high_pipe, dataloader, optimizer, device, text_features_all, img_features_all, config, eval_modality='eeg'):
    ...
    for batch_idx, (modal, data, labels, text, text_features, img, img_features, index, img_index, sub_ids) in enumerate(dataloader):
        ...
        if config.use_prior and epoch > int(prior_pct * config.epochs):     
            ...
            prior_loss = prior_criterion(noise_pre, noise) # (batch_size,)
    ...
    # Print the losses
    print(f"Epoch {epoch + 1} - Losses: Regresion Loss: {average_regress_loss:.4f}, Clip Loss: {average_loss_clip:.4f}, Prior Loss: {average_prior_loss:.4f}")
