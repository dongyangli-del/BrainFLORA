...
def train_model(epoch, unified_model, high_pipe, dataloader, optimizer, device, text_features_all, img_features_all, config, eval_modality='eeg'):
    unified_model.train()    
    ...
    # for batch_idx, (modal, data, labels, text, text_features, img, img_features, subject_id) in enumerate(dataloader):
        ...
        if config.use_prior and epoch > int(prior_pct * config.epochs):     
            prior_loss, prior_out = high_pipe.diffusion_prior(text_embed=upsamp_features, image_embed=img_features)                                    
        ...
        if epoch < int(mixup_pct * config.epochs):                
            loss_clip = mixco_nce(
                ...
                # img_loss = eeg_model.loss_func(neural_features_norm, img_features_norm, logit_scale)
                # loss_clip = img_loss
        else:
...
def main_train_loop(test_subjects, current_time, unified_model, high_pipe, train_dataloader, test_dataloader, optimizer, device, text_features_train_all, text_features_test_all, img_features_train_all, img_features_test_all, config, logger=None, eval_modality='eeg'):
    ...
    for epoch in range(config.epochs):
        # Train the model
        train_loss, train_accuracy = train_model(epoch, unified_model, high_pipe, train_dataloader, optimizer, device, text_features_train_all, img_features_train_all, config=config, eval_modality=eval_modality)
        if (epoch +1) % 10 == 0:                    
...
def main():
    ...
    parser.add_argument("--use_caption", action=argparse.BooleanOptionalAction, default=True, help="whether to train diffusion prior (True) or just rely on retrieval part of the pipeline (False)")
    ...
    unified_model = UnifiedEncoder(encoder_paths, device, num_experts=5, num_heads=args.depth, ff_dim=64*args.depth, num_layers=args.depth, use_caption=args.use_caption)
